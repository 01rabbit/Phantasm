...
class VesselRegistrySealTests(unittest.TestCase):
    def test_sealed_fields_round_trip_through_save_and_load(self):
    ...
    def test_cleartext_registry_never_holds_sealed_fields(self):
        ...
        with isolated_dirs() as (vs, _seal, config, _state):
            ...
            for sealed_key in (
                ...
                with self.subTest(sealed_key=sealed_key):
                    for entry in raw["vessels"]:
                        for face in entry.get("faces", []):
                            self.assertNotIn(sealed_key, face)
                    # And no stray copy anywhere in the file, including the
                    # vessel-level aggregate that used to sit alongside the
                    # per-Face figures.
                    self.assertNotIn(sealed_key, blob)

            self.assertNotIn(
                "ffff0000ffff0000", blob, "object fingerprint left in cleartext"
            )
            self.assertNotIn(
                "aGFzaGhhc2hoYXNoaGE9", blob, "destroy verifier left in cleartext"
            )
            self.assertNotIn("travel", blob, "Face label left in cleartext")

    def test_vessels_stay_listable_when_the_sidecar_is_missing(self):
    ...
    def test_a_sidecar_under_the_wrong_key_degrades_instead_of_raising(self):
    ...
    def test_a_truncated_sidecar_degrades_instead_of_raising(self):
        with isolated_dirs() as (vs, seal, _config, _state):
            vessel = "/tmp/demo.vessel"
            vs.register_vessel(vessel)
            vs.update_face_access(vessel, "face_a", file_count=12)
    ...
    def test_a_legacy_plaintext_registry_is_migrated_on_first_load(self):
    ...
    def test_emptying_the_registry_discards_the_sidecar(self):
        with isolated_dirs() as (vs, seal, _config, _state):
            vessel = "/tmp/demo.vessel"
            vs.register_vessel(vessel)
            vs.update_face_access(vessel, "face_a", file_count=12)
    ...
    def test_the_sidecar_is_owner_only(self):
        with isolated_dirs() as (vs, seal, _config, _state):
            vs.register_vessel("/tmp/demo.vessel")
            mode = os.stat(seal.VesselRegistrySeal().blob_path).st_mode & 0o777
            assert mode == 0o600

    def test_destroy_verifier_uses_the_argon2_memory_tier(self):
    ...
    def test_a_destroy_passphrase_set_under_legacy_parameters_still_verifies(self):
    ...
    def test_unusable_recorded_kdf_parameters_do_not_read_as_a_match(self):
        from phasmid.services.vessel_workflow_service import VesselWorkflowService
