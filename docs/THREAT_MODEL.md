...
- `vessel_registry.json` — **plaintext JSON at mode `0600` in the config directory.** Unlike the state directory blobs above, none of it is encrypted at rest. Per Face it records `file_count` and `occupancy`, the `dummy_profile` (which identifies the Face carrying generated filler), the `object_binding` perceptual fingerprints of the bound access object (`average_hash`, `edge_hash`, `brightness_histogram`, `color_histogram`, `threshold`), and `emergency_auth` — a `scrypt(n=2**14, r=8, p=1)` verifier for that Face's destroy passphrase.
- **This is an unresolved gap, not an accepted property.** It matters specifically against the in-scope physical-captor and coercing-authority adversaries, who by definition hold the device: the file is readable as the logged-in user with no passphrase and without launching Phasmid. It does not require the out-of-scope compromised-host assumption.
- Consequences worth stating plainly: the two-Face structure and each Face's volume are disclosed; the Face holding generated filler is identifiable from `dummy_profile`; the access object is characterisable from its stored fingerprints; and `emergency_auth` is an offline oracle that reveals whether a passphrase offered under coercion is the destroy credential, inverting the intent of the duress design.
- A purge leaves a signature here. `forget_face_contents()` deliberately resets content bookkeeping but preserves `object_binding` and `emergency_auth` as credentials, so a purged Face (bound, credentialed, zero files) stays distinguishable from a never-used one (unbound, uncredentialed, zero files). Destruction is therefore detectable from local state even though the bytes are gone.
- Encrypting these fields under the local state key is tracked separately. Until then, treat the config directory as sensitive local state on the same footing as the state directory, and include it in the seizure review checklist.
...
- `vessel_registry.json` is unencrypted (see [Configuration Directory Surface](#configuration-directory-surface)). Unlike the access-token leak above, this one is **not** accepted as residual: it discloses the two-Face structure, identifies which Face holds generated filler, characterises the bound access object, and provides an offline verifier for the destroy passphrase, all to an adversary who is explicitly in scope. It is a gap awaiting a fix, and the surrounding claims in this document should be read with that exception in mind.
