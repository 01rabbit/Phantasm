...
**EN:** "A quick note on where I come from: like a lot of people in this room, I started in uniform — a former Japan Ground Self-Defense Force officer, a Second Lieutenant, in a cyber unit. My work since has mostly been offensive — and offensive work is really about the **person, not just the algorithm**; the weakest link is human. Phasmid points that lesson the other way: **it doesn't protect the key, it protects the person holding it.** Day-to-day I'm a senior engineer at **GMO Cybersecurity by Ierae** in Tokyo, **working in the SOC** — defence, every day. Before that I was a **penetration tester**, CISSP; security research and open-source tools alongside — you may have seen the Azazel system or the PAKURI family at Black Hat Arsenal, BSides, SecTor, CODE BLUE. Find me at **01rabbit** on GitHub."
...
| 1 | **One file on disk** | 1:10 | TUI + シェル | **`ls` / `file` / `od` で実物を見せる。以降ファイル名で参照し続ける。** ここが全体の前提 |
