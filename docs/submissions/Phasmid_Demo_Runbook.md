...
- [ ] **端末幅を133桁以上にする**（`tput cols` で確認）。これを下回ると Expert フッタから
...
**最小端末幅: 133桁**（#160 で確定、#168 の `t Tokens` 追加で123→133に更新）
...
| 123桁以下 | `q Quit` も画面外 |
| 115桁以下 | `? Help` も画面外 |
| 107桁以下 | `t Tokens` も画面外 |

**投影端末は133桁以上を確保すること。** T-30 の設営時に確認する。
`l LUKS` は LUKS 無効時（既定）に非表示になったため、閾値は131桁から123桁へ下がった
（#160）。#168 で `t Tokens` バインディングを追加したことで133桁へ再度上がった。
