...
### 9.0.2 破壊経路の実機検証で出た3つの不具合（0.5.0・#190 で修正）
...
- **Step 4b（パスワードによる破壊）は実機で確認済み**（0.5.0・#191）。物体A＋破壊
...
- **明示的な `Clear this entry` パネル（`/destroy_face`）は実機で動作未確認。**
  復号成功後はカメラが解放される（`/retrieve` が `access_cue_service.close()` を呼ぶ）
  ため、そのままパネルを開いても物体が見えず弾かれる可能性がある。**壇上では使わない
  経路**なので本番には影響しないが、残している以上は要修正（→ §9 末尾「残課題」）。
...
- **`/destroy_face` パネルの実機動作**（上記）。壇上では使わないが、UI に残っている
  以上は動くべき。
- **レガシー経路の破壊セマンティクスが逆。** `_purge_for_password_role` は破壊
  パスワードで復号すると**中身を返した上でもう一方の Face を消す** — 0.5.0 で確定した
  規則（かざした Face 自身が消える／開示しない）と真逆。Vessel が1つも登録されて
  いないときにしか走らないため実害は出ないが、コードベースに逆向きの規則が併存
  している。
- **画像ファイルからの参照登録は 2段階撮影を経ていない**（上記）。
- **CLI と TUI の登録経路も 2段階撮影に未移行**（上記）。
