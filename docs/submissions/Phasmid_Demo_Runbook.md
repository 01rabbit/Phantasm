...
- **復号のたびにカメラが固まる。** 復元が成功すると `/retrieve` が
  `access_cue_service.close()` を呼んでいた（「省電力と発熱対策」）。**`close()` は
  全体停止の操作**で、マッチャースレッドを止め、まだ読んでいる者がいるかに関係なく
  消費者カウントを 0 にする。ブラウザのプレビューは最後のフレームのまま止まり、
  タブを移ると直る（新しいページが新しい `<img>` を作り、`/video_feed` を叩き直して
  `start()` が走るため）。**`/video_feed` が切断時に犯していたのと同じ誤り**で、
  向きが逆だっただけ。呼び出しを外した — 最後の消費者が抜けた時点で
  `generate_frames()` が自分でカメラを離すので、ページを離れたときには従来どおり
  解放される。回帰テストを追加（`test_a_successful_retrieval_leaves_the_camera_running`）。
