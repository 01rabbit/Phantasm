...
The system implements Janus Eidolon System (JES), a two-slot local disclosure architecture that separates visible disclosure from protected local state. It combines context profile templates, operator-supplied disclosure material with an optional free-space filler, Silent Standby transitions, and coercion-safe fallback behavior.
