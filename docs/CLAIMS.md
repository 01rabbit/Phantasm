...
| CLM-41 | WebUI pages, `/status`, and `/video_feed` require a page session established at `/unlock` with the access token; the mutation token is rendered only into pages served to a request that already holds one. | docs/THREAT_MODEL.md §WebUI Page Session | tests: `tests/test_web_server.py` | WebUI request authentication |
