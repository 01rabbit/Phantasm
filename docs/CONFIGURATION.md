...
| `PHASMID_CAMERA_SIZE` | `WIDTHxHEIGHT` | `640x480` | Camera | Capture resolution, which is also the resolution the cue is matched at. Was `320x240` — about 1.5% of what an `imx708` delivers — and it was the ceiling on everything downstream. Measured on a printed packet filling ~30% of the frame width, in focus: **24 template keypoints at 320x240, 572 at 640x480, 823 at 1024x768**; a template needs 60 of its own to bind at all. Higher costs frame time, and the match history needs several consecutive frames | `config.camera_frame_size()` |
| `PHASMID_CAMERA_MAX_EXPOSURE_US` | int (>=1000) | `33000` | Camera | Ceiling on the shutter, via `FrameDurationLimits`. Was effectively **200 ms**, and a person holding an object still is not still at 200 ms. Measured on a 572-keypoint template: a 3 px smear (~33 ms) scores 197 good matches, 9 px about 70, 21 px scores 22 and is refused. Shortening it is not free — the lost light returns as gain, and gain as noise, which measured *harsher* than blur | `config.camera_max_exposure_us()` |
| `PHASMID_CAMERA_DENOISE` | `minimal` / `off` / `fast` | `minimal` | Camera | How hard the ISP smooths before anything else sees the frame. Denoising removes small high-frequency structure — print, weave, the edge of a label — which is what ORB calls a corner. `fast` restores the ISP's own choice | `config.camera_denoise()` |
| `PHASMID_CAMERA_SHARPNESS` | float (`0.0`-`16.0`) | `1.5` | Camera | Edge contrast boost; `1.0` is the camera's default. FAST decides a corner from local contrast, so this raises the keypoint count on the same object. Overdone it manufactures edges out of noise | `config.camera_sharpness()` |
| `PHASMID_CAMERA_LOCK_AWB` | bool | `true` | Camera | Freeze the colour gains once the camera has settled. Grayscale is a weighted sum of the channels, so auto white balance hunting is a global change to what ORB sees — and on a NoIR sensor, whose red channel carries infrared the algorithm was not designed for, it hunts more than usual | `config.camera_lock_white_balance()` |
