...
A Vessel (`*.vessel`) is the concrete, operator-facing container that implements the Janus Eidolon two-slot model: each Vessel holds exactly two disclosure Faces (`face_a`/`face_b`, shown as Slot A/Slot B in the WebUI), each independently protected by its own passphrase and physical object cue. An operator can create and hold more than one Vessel at a time; `VesselWorkflowService` is the shared entry point both the TUI and the WebUI use for all Vessel and Face operations (create, delete, store, retrieve, list, remove).
